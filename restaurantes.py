...
for freguesia in freguesias:
    print(f"\n🔍 A pesquisar restaurantes em: {freguesia}")
    search_box = driver.find_element(By.ID, "searchboxinput")
    search_box.clear()
    search_box.send_keys(f"Restaurantes em {freguesia}" + Keys.ENTER)
    time.sleep(5)
    ...
    for i, link in enumerate(links):
        ...
        if (len(dados)) % 10 == 0:
            ...
            df.to_csv('restaurantes.csv', sep=';', index=False, encoding="utf-8-sig")
